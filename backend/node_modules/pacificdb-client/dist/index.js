...
var PacificDBClient = class {
  ...
  collection(name) {
    ...
    return {
      ...
      deleteOne: (filter) => this.request("POST", `/api/v1/db/collections/${encoded}/documents/delete-one`, { filter }, { isWrite: true }),
      drop: () => this.request(
        "POST",
        `/api/v1/db/collections/${encoded}/drop`,
        { confirmation: name, acknowledgeDataLoss: true },
        { isWrite: true }
      ),
      createIndex: (definition) => this.request("POST", `/api/v1/db/collections/${encoded}/indexes`, definition, { isWrite: true }),
      listIndexes: async () => {
        const r = await this.request("GET", `/api/v1/db/collections/${encoded}/indexes`, void 0, { isWrite: false });
        return r?.indexes ?? [];
      },
      dropIndex: (name2) => this.request("DELETE", `/api/v1/db/collections/${encoded}/indexes/${encodeURIComponent(name2)}`, void 0, { isWrite: true }),
      rebuildIndex: (name2) => this.request("POST", `/api/v1/db/collections/${encoded}/indexes/${encodeURIComponent(name2)}/rebuild`, {}, { isWrite: true }),
      validateIndexes: () => this.request("POST", `/api/v1/db/collections/${encoded}/indexes/validate`, {}, { isWrite: false })
    };
  }
  ...
  async request(method, path, body, opts = {}) {
    ...
    let refreshed = false;
    let lastError = null;
    const maxRetries = isWrite ? 0 : this.retryLimit;
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      ...
      try {
        ...
        if (error instanceof PacificDBClientError || error instanceof PacificDBAuthError || error instanceof PacificDBConfigError || error instanceof PacificDBServerError) {
          if (error instanceof PacificDBServerError && attempt < maxRetries) {
            lastError = error;
